...
public class MainService extends Service {
	...
	CorePlayerServiceConnection mCorePlayerConnection;
    ...
    public void onCreate() {
		...
		bindIntent.setClassName("com.android.music", "com.android.music.player.service.CorePlayerService");
		mCorePlayerConnection = new CorePlayerServiceConnection();
		mIsBound = this.bindService(bindIntent, mCorePlayerConnection, 0);
		
		if(mIsBound) {
			Logger.SysAppend(LogLevel.WARRNING, "bind successful", methodName);
		}
		
		mMediaReceiver = new MediaBroadcastReceiver(
				(AudioManager)getSystemService(Context. AUDIO_SERVICE), 
				sSongsMap, 
				mCorePlayerConnection
				); 
	...
	public void onDestroy() {
		...
		if (mIsBound) {
			this.unbindService(mCorePlayerConnection);
		}
